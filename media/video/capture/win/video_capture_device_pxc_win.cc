...
void VideoCaptureDevicePxcWin::DepthToGrayscaleRGB32(
  ...
  for (unsigned int i = 0; i < length; i++) {
    ...
    if (depth_value_in_millimeters < depth_range_in_millimeters_.min ||
        depth_value_in_millimeters > depth_range_in_millimeters_.max) {
      continue;
    }
...
void VideoCaptureDevicePxcWin::DepthToRawRGB32(
  ...
  for (unsigned int i = 0; i < length; i++) {
    // Layout is BGRA.
    rgb[i * kBytesPerPixelRGB32 + 2] = static_cast<uint8>(depth[i] & 0xFF);
    rgb[i * kBytesPerPixelRGB32 + 1] = static_cast<uint8>(depth[i] >> 8 & 0xFF);
  }
}
...
void VideoCaptureDevicePxcWin::DepthToAdaptiveRGB32(
  ...
  for (unsigned int i = 0; i < length; i++) {
    ...
    float ha = fmod(ld / (p / 2), 2);
    if (ha > 1.0)
      ha = 2.0 - ha;

    float hb = fmod((ld - p / 4) / (p / 2), 2);
    if (hb > 1.0)
      hb = 2.0 - hb;
